...
import { UnsupportedContentCenter } from './admin/UnsupportedContent'
import {
...
const navigation = [
  ...
  { label: '데이터 및 전환', items: [['sources', Database, '데이터 원본'], ['migration', RefreshCw, 'Migration Center'], ['exceptions', CircleAlert, '예외 콘텐츠']] },
    ...
    <div className={`admin-layout ${navOpen ? 'nav-open' : ''}`}>
      ...
      <main>
        <Routes>
          ...
          <Route path="exceptions" element={<UnsupportedContentCenter notify={notify} confirm={confirm} />} />
...
function MigrationCenter() {
  ...
  const [busy, setBusy] = useState(false)
  const [error, setError] = useState('')
  const load = useCallback(async () => { const [dashboard, jobList, macroList, unsupportedResult] = await Promise.all([api('/api/v1/admin/migration'), api('/api/v1/admin/migration/jobs'), api('/api/v1/admin/migration/macros'), api('/api/v1/admin/migration/unsupported?status=OPEN&limit=12')]); setData(dashboard); setJobs(jobList || []); setMacros(macroList || []); setUnsupported(unsupportedResult.items || []); setError('') }, [])
  ...
  const reconcile = () => act(() => api('/api/v1/admin/migration/reconciliation', { method: 'POST' }), '독립 정합성 검증을 시작했습니다.')
  ...
  const completedSnapshot = jobs.some(job => job.kind === 'SNAPSHOT' && ['COMPLETE', 'COMPLETED_WITH_ERRORS'].includes(job.status))
  return <>
    <AdminHeader eyebrow="CONFLUENCE MIGRATION CENTER" title="안전한 데이터 전환" description="Legacy 원본을 수정하지 않고 Discovery, Initial Snapshot, 독립 정합성 검증을 실행합니다." action={<><button className="button secondary" onClick={discover} disabled={busy || !!active}><FileSearch /> Schema Discovery</button><button className="button secondary" onClick={reconcile} disabled={busy || !completedSnapshot || !!active}><ListChecks /> 정합성 검증</button><button className="button primary" onClick={snapshot} disabled={busy || !discovery || !!active}><RefreshCw /> Initial Snapshot</button></>} />
