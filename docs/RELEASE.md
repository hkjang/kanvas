...
`VERSION`이 `0.3.0`이면 Git tag는 `v0.3.0`, Docker image 이름은 `kanvas-v0.3.0:latest`, GitHub Release asset은 `kanvas-v0.3.0.tar.gz`입니다.
...
make test
make release-tar
docker load < dist/kanvas-v0.3.0.tar.gz
```

## GitHub

```bash
git tag -a v0.3.0 -m 'Kanvas v0.3.0'
